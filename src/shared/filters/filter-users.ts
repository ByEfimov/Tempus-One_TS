...
export function filterUsers(filter: string, array?: OpenUserType[]) {
  ...
  if (filter) {
    return filteredArray?.filter((user) => user.name.toLocaleLowerCase().includes(filter.toLocaleLowerCase()));
  }
  return array;
}
...
export function filterTeams(filter: string, array?: OpenTeamType[]) {
  if (filter) {
    return array?.filter((team) => team.title.toLocaleLowerCase().includes(filter.toLocaleLowerCase()));
  }
  return array;
}
