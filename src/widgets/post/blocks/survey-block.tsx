...
const SurveyBlock = ({ block, selectEditMode }: { block: blockType; selectEditMode: (blockId: number) => void }) => {
  ...
  return (
    ...
    >
      ...
      <BottomInfo block={block}>
        <motion.li onClick={() => dispatch(addVariant({ blockId: block.id }))} variants={defaultItem}>
      ...
      <>
        ...
        <motion.ul {...formContainer} className={Styles.variants}>
          {block.data.variants?.map((variant) => (
            <motion.li key={variant.id} variants={formItem} className={Styles.variant}>
              <input
                type="text"
                value={variant.text}
                onChange={(e) =>
