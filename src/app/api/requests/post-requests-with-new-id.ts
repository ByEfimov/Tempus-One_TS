...
export async function postRequestWithNewId<T>(Path: string, NewData: T): Promise<string> {
...
export async function postRequestWithoutNewId<T>(Path: string, NewData: T): Promise<string> {
  const db = getDatabase();
  const postListRef = ref(db, Path);
